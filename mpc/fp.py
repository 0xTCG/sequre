...
class FP:
    ...
    def normalizer_even_exp(self: 'FP', a: np.ndarray, field: int) -> tuple:
        ...
        # TODO: Impelment np.sum over axis=1
        for i in range(n):
    ...
    def fp_sqrt(self: 'FP', a: np.ndarray, field: int = param.BASE_P) -> tuple:
        ...
        h_and_g[0] = self.trunc(
            h_and_g[0], param.NBIT_K // 2 + param.NBIT_F + 2, (param.NBIT_K - param.NBIT_F) // 2 + 1)

        h_and_g[1][0][:] = add_mod(h_and_g[0][0], h_and_g[0][0], field)
        h_and_g[1][0][:] = self.arithmetic.multiply(h_and_g[1][0], a, elem_wise=True)
        h_and_g[1] = self.trunc(h_and_g[1], k = param.NBIT_K + param.NBIT_F, m = param.NBIT_F)
        ...
        for _ in range(niter):
            ...
            r = self.trunc(r, k = param.NBIT_K + param.NBIT_F, m = param.NBIT_F)
            r = np.mod(-r, field)
            if self.pid == 1:
            ...
            h_and_g[0] = self.trunc(h_and_g[0], k = param.NBIT_K + param.NBIT_F, m = param.NBIT_F)
            h_and_g[1] = self.trunc(h_and_g[1], k = param.NBIT_K + param.NBIT_F, m = param.NBIT_F)
