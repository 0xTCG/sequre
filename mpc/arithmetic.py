...
class Arithmetic:
    ...
    def beaver_partition(self: 'Arithmetic', x: np.ndarray, field: int) -> tuple:
    ...
    def add_public(self: 'Arithmetic', x: np.ndarray, a: np.ndarray, field: int) -> np.ndarray:
        if self.pid == 1:
    ...
    def beaver_mult(
            ...
            y_r: np.ndarray, r_2: np.ndarray, elem_wise: bool, field: int) -> np.ndarray:
        ...
        xy = mul_func(x_r, r_2)
        xy = add_func(xy, mul_func(r_1, y_r))
        if self.pid == 1:
            xy = add_func(xy, mul_func(x_r, y_r))

        return xy

    def beaver_reconstruct(self: 'Arithmetic', elem: np.ndarray, field: int) -> np.ndarray:
    ...
    def multiply(self: 'Arithmetic', a: np.ndarray, b: np.ndarray, elem_wise: bool, field: int) -> np.ndarray:
    ...
    def beaver_partition_bulk(self: 'Matrix', x: list, field: int) -> tuple:
    ...
    def beaver_reconstruct_bulk(self: 'Matrix', x: list, field: int) -> tuple:
        # TODO: Do this in parallel
        return [self.beaver_reconstruct(e, field) for e in x]

    def mult_aux_parallel(self: 'MPCEnv', a: list, b: list, elem_wise: bool, field: int) -> list:
        # TODO: Vectorize this method. Make it parallel by having a and b as ndarrays.
        # It will probably be reduced to multiply above then.
        assert len(a) == len(b)
        nmat: int = len(a)
    ...
    def mult_mat_parallel(self: 'MPCEnv', a: list, b: list, field: int) -> list:
        # TODO: Vectorise/parallelize this method
        return self.mult_aux_parallel(a, b, False, field)
    
    def beaver_inner_prod(self: 'MPCEnv', ar: np.ndarray, am: np.ndarray, field: int) -> int:
    ...
    def beaver_inner_prod_pair(
            self: 'MPCEnv', ar: np.ndarray, am: np.ndarray, br: np.ndarray, bm: np.ndarray) -> int:
        # TODO: This method is not validated.
        ab: int = 0
    ...
    def inner_prod(self: 'MPCEnv', a: np.ndarray, field: int) -> np.ndarray:
        # TODO: This method is not validated.
