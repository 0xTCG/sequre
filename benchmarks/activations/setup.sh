...
find_python312() {
    ...
    if command -v conda >/dev/null 2>&1; then
        if ! conda env list | grep -q '^sequre-act-bench '; then
            conda create -y -n sequre-act-bench python=3.12 >/dev/null
        fi
        conda run -n sequre-act-bench python -c 'import sys; print(sys.executable)'
        return
    fi
