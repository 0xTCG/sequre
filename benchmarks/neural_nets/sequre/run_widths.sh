#!/usr/bin/env bash
# Runs the Sequre neural-network benchmark at more than one share width so the
# report can show them side by side.
#
...
#   ./run_widths.sh 64 128 192 # include the 64-bit width-matched control
#
# 64 is deliberately NOT in the default set. It exists to match the 64-bit ring
# CrypTen uses -- same 16 fractional bits -- so that the speed comparison is
# like-for-like, and it carries only 10 bits of statistical security. See
# README.md, "Matching CrypTen's 64-bit width".
set -euo pipefail
