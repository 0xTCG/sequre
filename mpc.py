...
class MPCEnv:
    ...
    def lagrange_interp(self: 'MPCEnv', x: Vector, y: Vector, fid: int = 0) -> Vector:
        ...
        denom_inv = Vector([Zp(e, base=self.primes[fid]) for e in denom_inv])
        numer = numer.mult(denom_inv)
        numer.set_field(self.primes[fid])
    ...
    def fan_in_or(self: 'MPCEnv', a: Matrix, fid: int) -> Vector:
        ...
        coeff[0] = self.or_lagrange_cache[key]  # Be careful for it is passed by reference
    ...
    def beaver_partition_bulk(self: 'Matrix', x: Vector, fid: int = 0) -> tuple:
        return self.beaver_partition(x)

    def mult_aux_parallel(self: 'MPCEnv', a: Vector, b: Vector, elem_wise: bool, fid: int = 0) -> Vector:
        assert len(a) == len(b)
        nmat: int = len(a)
        ...
        c = Vector([Matrix()] * nmat)
        for k in range(nmat):
            c[k] = self.beaver_mult(ar[k], am[k], br[k], bm[k], elem_wise, fid)

        c = self.beaver_reconstruct(c, fid)

    def mult_mat_parallel(self: 'MPCEnv', a: Vector, b: Vector, fid: int = 0) -> Vector:
    ...
    def prefix_or(self: 'MPCEnv', a: Matrix, fid: int) -> Matrix:
        ...
        # Zero-pad to L2 bits
        a_padded = Matrix().from_value(a)
        a_padded.set_dims(n, L2)

        print(f'a_padded before reshape', a_padded)
        self.reshape(a_padded, n * L, L)
        print(f'a_padded after reshape', a_padded)
        ...
        if self.pid > 0:
            for i in range(n):
                for j in range(L):
                    tmp[i][j] = Vector(a_padded[L * i + j].value)
    ...
    def fp_div(self: 'MPCEnv', a: Vector, b: Vector) -> Vector:
        ...
        if n > param.DIV_MAX_N:
            ...
            for i in range(nbatch):
                ...
                for j in range(batch_size):
                    a_copy[j] = a[start + j] * Zp(1)
                    b_copy[j] = b[start + j] * Zp(1)

                c_copy: Vector = self.fp_div(a_copy, b_copy)
                for j in range(batch_size):
                    c[start + j] = c_copy[j] * Zp(1)
            return c
        ...
        b_scaled: Vector = self.mult_elem(b, s)
        self.trunc_vec(b_scaled, param.NBIT_K, param.NBIT_K - param.NBIT_F)

        b_scaled_sq: Vector = self.mult_elem(b_scaled, b_scaled)
        ...
        w: Vector = self.mult_elem(scaled_est, s)
        ...
        x: Vector = self.mult_elem(w, b)
        self.trunc_vec(x)

        one: Zp = Zp(1)  # self.int_to_fp(1, Param::NBIT_K, Param::NBIT_F);
        ...
        for i in range(niter):
            ...
            y: Vector = self.beaver_mult_elem(yr, ym, xpr, xm)
            x: Vector = self.beaver_mult_elem(xr, xm, xr, xm)
    ...
    def share_random_bits(self: 'MPCEnv', k: int, n: int, fid: int) -> tuple:
        if self.pid == 0:
            ...
            rbits_mask: Matrix = self.rand_mat(n, k, fid)
            self.restore_seed(1)
            ...
            rbits: Matrix = self.receive_matrix(0)
        else:
            self.switch_seed(0)
            r: Vector = self.rand_vector(n)
            rbits: Matrix = self.rand_mat(n, k, fid)
            self.restore_seed(0)
    ...
    def normalizer_even_exp(self: 'MPCEnv', a: Vector) -> tuple:
        ...
        if self.pid > 0:
            for i in range(n):
                ...
                for j in range(1, param.NBIT_K + 1):
                    ...
                    if self.pid == 1:
                       ep[i][j] += ebits[i][j - 1]
            ep.set_field(self.primes[fid])

        E: Matrix = self.prefix_or(ep, fid)

        tpneg = Matrix(n, param.NBIT_K)
        if self.pid > 0:
            for i in range(n):
                for j in range(param.NBIT_K):
                    tpneg[i][j] = E[i][j] - rbits[i][j] * (1 - ebits[i][j])
            tpneg.set_field(self.primes[fid])

        Tneg: Matrix = self.prefix_or(tpneg, fid)
        half_len: int = param.NBIT_K // 2

        efir = Matrix(n, param.NBIT_K)
        rfir = Matrix(n, param.NBIT_K)
        if self.pid > 0:
            efir = self.mul_elem(ebits, Tneg)
            efir.set_field(self.primes[fid])
        rfir = self.mult_elem(rbits, Tneg, fid)

        double_flag: Vector = self.less_than_bits(efir, rfir, fid)

        odd_bits = Matrix(n, half_len)
        even_bits = Matrix(n, half_len)
        if self.pid > 0:
            for i in range(n):
                for j in range(half_len):
                    ...
                    if ((2 * j + 2) < param.NBIT_K):
                        ...
                        even_bits[i][j] = 0
            odd_bits.set_field(self.primes[fid])
            even_bits.set_field(self.primes[fid])
        ...
        for i in range(n):
            ...
            if self.pid == 1:
                odd_bit_sum[i] += 1
                even_bit_sum[i] += 1
        odd_bit_sum.set_field(self.primes[fid])
        even_bit_sum.set_field(self.primes[fid])
        ...
        if self.pid != 0:
            ...
            diff.set_field(self.primes[fid])

        diff: Vector = self.mult_elem(double_flag, diff, fid)
    ...
    def less_than_bits_aux(self: 'MPCEnv', a: Matrix, b: Matrix, public_flag: int, fid: int) -> Vector:
        ...
        # TODO: optimize
        f_arr = Vector([Matrix()] * n)
        b_arr = Vector([Matrix()] * n)
        for i in range(n):
            f_arr[i].set_dims(1, L)
            b_arr[i].set_dims(L, 1)
        ...
        c_arr: Vector = self.mult_mat_parallel(f_arr, b_arr, fid)
