...
from utils.utils import get_cache_path, get_output_path, get_temp_path, rand_int
...
def gwas_protocol(mpc: MPCEnv) -> bool:
    ...
    # Useful constants
    two = Tp(2)  # Zp(2, base=param.BASE_P)
    twoinv = TypeOps.mod_inv(two)
    fp_one = TypeOps.double_to_fp(1.0, param.NBIT_K, param.NBIT_F)

    pheno = [TP(0) for _ in range(n0)]  # Vector([Tp()  # Zp(0, base=param.BASE_P) for _ in range(n0)])
    cov = [[TP(0) for _ in range(param.NUM_COVS)] for _ in range(n0)]  # Matrix(n0, param.NUM_COVS)

    print("Initial data sharing results found")

    with open(get_cache_path(mpc.pid, "input_pheno_cov")) as f:
        pheno = read_vector(f, n0)
        cov = mpc.read_matrix(f, n0, param.NUM_COVS)

    print("Phenotypes and covariates loaded")

    gkeep1 = [TP(0) for _ in range(m0)]  # Vector([Tp()  # Zp(0, base=param.BASE_P) for _ in range(m0)])
        ...
        if history:
            ...
            if mpc.pid == 2:
                with open(get_output_path(mpc.pid, "gkeep1")):
                    for i, line in zip(range(m0), f):
                        gkeep1[i] = Tp(line)  # Zp(int(f.readline()), base=param.BASE_P)

                mpc.comms.send_data(gkeep1, 0)
                mpc.comms.send_data(gkeep1, 1)
            else:
                gkeep1 = mpc.comms.receive_vector(2, msg_len=TypeOps.get_vec_len(m0), fid=0)
        else:
            gmiss = [TP(0) for _ in range(m0)]  # Vector([Tp()  # Zp(0, base=param.BASE_P) for _ in range(m0)])
            ...
            if os.path.exists(get_cache_path(mpc.pid, "gmiss")):
                ...
                if mpc.pid > 0:
                    with open(get_cache_path(mpc.pid, "input_geno")) as f:
                        ...
                        for i in range(n0):
                            ...
                            tmp_mat = rand_mat([3, m0])  # Matrix(3, m0, randomise=True)  # g_mask
                            miss_mask = rand_vec(m0, param.BASE_P)
                            mpc.prg.restore_seed(10)

                            if mpc.pid == 2:
                                skip_data(f, 3, m0)
                                miss = read_vector(f, m0)
                ...
                with open(get_cache_path(mpc.pid, "gmiss")) as f:
                    write_to_file(gmiss, f)

                print("Wrote results to cache")

            print("Locus missing rate filter ... ")

            gmiss_ub = Tp(n0 * param.GMISS_UB) % param.BASE_P  # Zp(n0 * param.GMISS_UB, base=param.BASE_P)
            gkeep1 = mpc.boolean.less_than_public(gmiss, gmiss_ub)
            gkeep1 = mpc.comms.reveal_sym(gkeep1)

            if mpc.pid == 2:
                mpc.comms.send_data(gkeep1, 0)
            elif mpc.pid == 0:
                gkeep1 = mpc.comms.receive_vector(2, TypeOps.get_vec_len(m0))
    ...
    snp_pos = mpc.filter(snp_pos, gkeep1)

    ikeep = [TP(0) for _ in range()]  # Vector([Tp()  # Zp(0, base=param.BASE_P) for _ in range(n0)])
        ...
        if history:
            ...
            if mpc.pid == 2:
                with open(get_output_path(mpc.pid, "ikeep")) as f:
                    for i, line in zip(range(n0), f):
                        ikeep[i] = Tp(line)  # Zp(int(f.readline()), base=param.BASE_P)

                mpc.comms.send_data(ikeep, 0)
                mpc.comms.send_data(ikeep, 1)
            else:
                receive_vector(ikeep, 2, TypeOps.get_vec_len(n0))
        else:
            imiss = [TP(0) for _ in range(n0)]  # Vector([Tp()  # Zp(0, param.BASE_P) for _ in range(n0)])
            ihet = [TP(0) for _ in range(n0)]  # Vector([Tp()  # Zp(0, param.BASE_P) for _ in range(n0)])
            ...
            if os.path.exists(get_cache_path(mpc.pid, "imiss_ihet")):
                ...
                with open(get_cache_path(mpc.pid, "imiss_ihet")) as f:
                    imiss = read_vector(f, n0)
                    ihet = read_vector(f, n0)
            else:
                ...
                if mpc.pid > 0:
                    with open(get_cache_path(mpc.pid, "input_geno")) as f:
                        ...
                        for i in range(n0):
                            # Load stored Beaver partition
                            mpc.prg.switch_seed(10)
                            g_mask = [[TP() for _ in range()] for _ in range()]  # Matrix(3, m0, randomise=True)
                            miss_mask = mpc.random_vector(m0)
                            mpc.prg.restore_seed(10)
                ...
                with open(get_cache_path(mpc.pid, "imiss_ihet")) as f:
                    write_to_file(imiss, f)
                    write_to_file(ihet, f)

                print("Wrote results to cache")

            # Individual missingness filter
            print("Individual missing rate filter ... ")
            imiss_ub: Zp = Tp()  # Zp(m1 * param.IMISS_UB, base=param.BASE_P)
            ...
            m1_obs = [TP(0) for _ in range()]  # Vector([Tp()  # Zp(0, base=param.BASE_P) for _ in range(n0)])
            if mpc.pid > 0:
                for i in range(n0):
                    m1_obs[i] = -imiss[i]
                    if mpc.pid == 1:
            ...
            ihet_ub = [TP(0) for _ in range()]  # Vector([Tp()  # Zp(0, base=param.BASE_P) for _ in range(n0)])
            ihet_lb = [TP(0) for _ in range()]  # Vector([Tp()  # Zp(0, base=param.BASE_P) for _ in range(n0)])
            ...
            het_filt = mpc.less_than(ihet, ihet_ub)
            tmp_vec = mpc.not_less_than(ihet, ihet_lb)
            het_filt = mpc.mult_vec(het_filt, tmp_vec)

            ikeep = mpc.mult_vec(ikeep, het_filt)

            # Reveal samples to be filtered
            mpc.comms.reveal_sym(ikeep)

            if mpc.pid == 2:
                mpc.comms.send_data(ikeep, 0)
            elif mpc.pid == 0:
                mpc.comms.receive_vector(ikeep, 2, TypeOps.get_vec_len(n0))
    ...
    n1: int = int(sum(ikeep, Tp()  # Zp(0, base=param.BASE_P)))

    print("Filtering phenotypes and covariates")
    pheno = mpc.filter(pheno, gkeep1)
    cov: Matrix = mpc.filter_rows(cov, ikeep)

    ctrl = mpc.flip_bit(pheno)

    ctrl, ctrl_mask = mpc.beaver_partition(ctrl, fid=0)

    dosage_sum = [TP(0) for _ in range()]  # Vector([Tp()  # Zp(0, param.BASE_P) for _ in range(m1)])
    gmiss = [TP(0) for _ in range()]  # Vector([Tp()  # Zp(0, param.BASE_P) for _ in range(m1)])
    gmiss_ctrl = [TP(0) for _ in range()]  # Vector([Tp()  # Zp(0, param.BASE_P) for _ in range(m1)])
    dosage_sum_ctrl = [TP(0) for _ in range()]  # Vector([Tp()  # Zp(0, param.BASE_P) for _ in range(m1)])
    g_count_ctrl = [[TP() for _ in range()] for _ in range()]  # Matrix(3, m1)
    n1_ctrl = Tp()  # Zp(0, param.BASE_P)
    ...
    if (os.path.exists(get_cache_path(mpc.pid, "geno_stats"))):
        ...
        with open(get_cache_path(mpc.pid, "geno_stats")) as f:
            gmiss = read_vector(f, m1, fid=0)
            gmiss_ctrl = read_vector(f, m1, fid=0)
            dosage_sum = read_vector(f, m1, fid=0)
            dosage_sum_ctrl = read_vector(f, m1, fid=0)
            g_count_ctrl: Matrix = mpc.read_matrix(f, 3, m1, fid=0)
            n1_ctrl = Tp()  # Zp(int(f.readline()), param.BASE_P)
    else:
        ...
        with open(get_cache_path(mpc.pid, "input_geno")) as f:
            if mpc.pid > 0:
                ...
                for p in range(1, 3):
                    mpc.prg.import_seed(10 + p, int(f.readline()))
            ...
            g = list()
            g_mask = list()
            dosage = [[TP() for _ in range()] for _ in range()]  # Matrix(bsize, m1)
            dosage_mask = [[TP() for _ in range()] for _ in range()]  # Matrix(bsize, m1)
            miss = [[TP() for _ in range()] for _ in range()]  # Matrix(bsize, m1)
            miss_mask = [[TP() for _ in range()] for _ in range()]  # Matrix(bsize, m1)

            for k in range(3):
                g.append([[TP() for _ in range()] for _ in range()]  # Matrix(bsize, m1))
                g_mask.append([[TP() for _ in range()] for _ in range()]  # Matrix(bsize, m1))

            ctrl_vec = [TP(0) for _ in range()]  # Vector([Tp()  # Zp(0, param.BASE_P) for _ in range(bsize)])
            ctrl_mask_vec = [TP(0) for _ in range()]  # Vector([Tp()  # Zp(0, param.BASE_P) for _ in range(bsize)])
            ...
            for i in range(n1):
                ...
                g0 = [[TP() for _ in range()] for _ in range()]  # Matrix()
                g0_mask = [[TP() for _ in range()] for _ in range()]  # Matrix()
                miss0 = [TP(0) for _ in range()]  # Vector()
                miss0_mask = [TP(0) for _ in range()]  # Vector()
                ...
                while (ikeep[ind] != 1):
                    if mpc.pid > 0:
                        ...
                        mpc.prg.switch_seed(10)
                        g0_mask = [[TP() for _ in range()] for _ in range()]  # Matrix(3, m0, randomise=True)
                ...
                if mpc.pid > 0:
                    ...
                    g0 = [[TP() for _ in range()] for _ in range()]  # Matrix(3, m0)
                    g0_mask = [[TP() for _ in range()] for _ in range()]  # Matrix(3, m0)
                    miss0 = [TP(0) for _ in range()]  # Vector([Tp()  # Zp(0, param.BASE_P) for _ in range(m0)])
                    miss0_mask = [TP(0) for _ in range()]  # Vector([Tp()  # Zp(0, param.BASE_P) for _ in range(m0)])

                    for p in range(1, 3):
                        mpc.prg.switch_seed(10 + p)
                        tmp_mat = [[TP() for _ in range()] for _ in range()]  # Matrix(3, m0, randomise=True)
            ...
            if (i % bsize == bsize - 1 or i == n1 - 1):
                ...
                gmiss_ctrl = mpc.beaver_mult_vec(
                    ctrl_vec, ctrl_mask_vec, miss, miss_mask)
                dosage_sum_ctrl = mpc.beaver_mult_vec(
                    ctrl_vec, ctrl_mask_vec, dosage, dosage_mask)
                for k in range(3):
                    g_count_ctrl[k] = mpc.beaver_mult_vec(ctrl_vec, ctrl_mask_vec, g[k], g_mask[k])

        gmiss_ctrl = mpc.beaver_reconstruct(gmiss_ctrl)
        dosage_sum_ctrl = mpc.beaver_reconstruct(dosage_sum_ctrl)
        g_count_ctrl = mpc.beaver_reconstruct(g_count_ctrl)

        # Write to cache
        with open(get_cache_path(mpc.pid, "geno_stats")) as f:
            write_to_file(gmiss, f)
            write_to_file(gmiss_ctrl, f)
            write_to_file(dosage_sum, f)
            write_to_file(dosage_sum_ctrl, f)
            write_to_file(g_count_ctrl, f)
            write_to_file(n1_ctrl, f)
    ...
    dosage_tot = [TP(0) for _ in range()]  # Vector()
    dosage_tot_ctrl = [TP(0) for _ in range()]  # Vector()
    if mpc.pid > 0:
        dosage_tot = -gmiss
        dosage_tot_ctrl = -gmiss_ctrl
        dosage_tot += Tp()  # Zp(n1, param.BASE_P)
    ...
    maf = [TP(0) for _ in range()]  # Vector()
    maf_ctrl = [TP(0) for _ in range()]  # Vector()
    if (os.path.exists(get_cache_path(mpc.pid, "maf"))):
        print("maf cache found")
        with open(get_cache_path(mpc.pid, "maf")) as f:
            maf = read_vector(f, dosage_tot.length())
            maf_ctrl = read_vector(f, dosage_tot_ctrl.length())
    else:
        maf = mpc.fp_div(dosage_sum, dosage_tot)
        maf_ctrl = mpc.fp_div(dosage_sum_ctrl, dosage_tot_ctrl)

        with open(get_cache_path(mpc.pid, "maf"), 'wb') as f:
            write_to_file(maf, f)
            write_to_file(maf_ctrl, f)
    print("done. ")

    Maf = [TP(0) for _ in range()]  # Vector()  # MAJOR allele freq
    maf_ctrl = [TP(0) for _ in range()]  # Vector()
    if mpc.pid > 0:
        Maf = -maf
        Maf_ctrl = -maf_ctrl
    ...
    g_var_bern = mpc.mult_vec(maf, Maf)
    mpc.trunc_vec(g_var_bern)

    gkeep2 = [TP(0) for _ in range()]  # Vector([Tp()  # Zp(0, param.BASE_P) for _ in range(m1)])
    ...
    if param.SKIP_QC:
        ...
        if history:
            ...
            if mpc.pid == 2:
                with open(get_output_path(mpc.pid, "gkeep2")) as f:
                    for i in range(m1):
                        gkeep2[i] = Tp()  # Zp(int(f.readline()), param.BASE_P)

                mpc.comms.send_data(gkeep2, 0)
                mpc.comms.send_data(gkeep2, 1)
            else:
                mpc.comms.receive_vector(gkeep2, 2, TypeOps.get_vec_len(m1))
        else:
            gkeep2 = mpc.boolean.less_than_public(maf, maf_ub)
            tmp_vec = mpc.not_less_than_public(maf, maf_lb)
            gkeep2 = mpc.mult_vec(gkeep2, tmp_vec)
            ...
            g_exp_ctrl = [[TP() for _ in range()] for _ in range()]  # Matrix(3, m1)
        ...
        hwe_chisq = [TP(0) for _ in range()]  # Vector([Tp()  # Zp(0, param.BASE_P) for _ in range(m1)])

        if (os.path.exists(get_cache_path(mpc.pid, "hwe"))):
            print("HWE cache found")
            with open(get_cache_path(mpc.pid, "hwe")) as f:
                hwe_chisq = read_vector(f, m1)
        else:
            for i in range(3):
                diff = [Tp()  # Zp(0, param.BASE_P) for _ in range(m1)]
                if mpc.pid > 0:
    ...
    m2: int = int(sum(gkeep2, Tp()  # Zp(0, param.BASE_P)))
    print("n1: ", n1, ", ", "m2: ", m2)
    ...
    g_std_bern_inv = [TP(0) for _ in range()]  # Vector()
    if (os.path.exists(get_cache_path(mpc.pid, "stdinv_bern"))):
        ...
        with open(get_cache_path(mpc.pid, "stdinv_bern")) as f:
            g_std_bern_inv = read_vector(f, len(g_var_bern))
        ...
        with open(get_cache_path(mpc.pid, "stdinv_bern"), 'wb') as f:
            write_to_file(g_std_bern_inv, f)

    g_mean = [TP(0) for _ in range()]  # Vector([Tp()  # Zp(0, param.BASE_P) for _ in range(m2)])
    if mpc.pid > 0:
        g_mean = maf * 2
    ...
    if (os.path.exists(get_cache_path(mpc.pid, "pca_input"))):
        ...
        with open(get_cache_path(mpc.pid, "input_geno")) as f_geno:
            if mpc.pid > 0:
                mpc.prg.import_seed(10, f_geno)
            else:
                for p in range(1, 3):
                    mpc.prg.import_seed(10 + p, f_geno)
            ...
            with open(get_cache_path(mpc.pid, "pca_input")) as f_pca:
                ...
                for i in range(n1):
                    ...
                    # Filter out loci that failed missing rate filter
                    g = [[TP() for _ in range()] for _ in range()]  # Matrix(3, m3)
                    g_mask = [[TP() for _ in range()] for _ in range()]  # Matrix(3, m3)
                    miss = [TP(0) for _ in range()]  # Vector([Tp()  # Zp(0, param.BASE_P) for _ in range(m3)])
                    miss_mask = [TP(0) for _ in range()]  # Vector([Tp()  # Zp(0, param.BASE_P) for _ in range(m3)])
                    ind2: int = 0
                    for j in range(m0):
                        if gkeep3[j]:
                            for k in range(3):
    ...
    g_mean_pca = mpc.filter(g_mean, selected)
    g_stdinv_pca = mpc.Filter(g_std_bern_inv, selected)

    g_mean_pca_mask = mpc.beaver_partition(g_mean_pca)
    g_stdinv_pca_mask = mpc.beaver_partition(g_stdinv_pca)

    # Pass 2: Random sketch
    Y_cur = [[TP() for _ in range()] for _ in range()]  # Matrix(kp, m3)
    ...
    if (os.path.exists(get_cache_path(mpc.pid, "sketch"))):
        ...
        Y_cur_adj = [[TP() for _ in range()] for _ in range()]  # Matrix(kp, m3)
    ...
    gQ = [[TP() for _ in range()] for _ in range()]  # Matrix()
    ...
    if (os.path.exists(get_cache_path(mpc.pid, "piter"))):
        ...
        Y = [[TP() for _ in range()] for _ in range()]  # Matrix(kp, m3)
        ...
        gQ_adj = [[TP() for _ in range()] for _ in range()]  # Matrix()
        Q_mask = [[TP() for _ in range()] for _ in range()]  # Matrix()
        Q_scaled = [[TP() for _ in range()] for _ in range()]  # Matrix()
        Q_scaled_mask = [[TP() for _ in range()] for _ in range()]  # Matrix()
        Q_scaled_gmean = [[TP() for _ in range()] for _ in range()]  # Matrix()
        Q_scaled_gmean_mask = [[TP() for _ in range()] for _ in range()]  # Matrix()
        ...
        for pit in range(param.NUM_POWER_ITER + 1):
            ...
            Q_scaled = [[TP() for _ in range()] for _ in range()]  # Matrix(kp, m3)
            for i in range(kp):
            ...
            Q_scaled_gmean = [[TP() for _ in range()] for _ in range()]  # Matrix(kp, m3)
            for i in range(kp):
            ...
            g = [[TP() for _ in range()] for _ in range()]  # Matrix(bsize, m3)
            g_mask = [[TP() for _ in range()] for _ in range()]  # Matrix(bsize, m3)
            miss = [[TP() for _ in range()] for _ in range()]  # Matrix(bsize, m3)
            miss_mask = [[TP() for _ in range()] for _ in range()]  # Matrix(bsize, m3)
            
            # Pass 1
            gQ = [[TP() for _ in range()] for _ in range()]  # Matrix(n1, kp)
            gQ_adj = [[TP() for _ in range()] for _ in range()]  # Matrix(n1, kp)
            ...
            with open(get_cache_path(mpc.pid, "pca_input")) as f:
                for cur in range(n1):
                    ...
                    if cur % bsize == bsize - 1:
                        tmp_mat = [[TP() for _ in range()] for _ in range()]  # Matrix(bsize, kp)
            ...
            if remainder > 0:
                ...
                tmp_mat = [[TP() for _ in range()] for _ in range()]  # Matrix(remainder, kp)
                tmp_mat = mpc.beaver_mult(miss, miss_mask, Q_scaled_gmean, Q_scaled_gmean_mask)
                for i in range(remainder):
            ...
            gQ = [[TP() for _ in range()] for _ in range()]  # Matrix(kp, m3)
            gQ_adj = [[TP() for _ in range()] for _ in range()]  # Matrix(kp, m3)
            ...
            Qsub = [[TP() for _ in range()] for _ in range()]  # Matrix(bsize, kp)
            Qsub_mask = [[TP() for _ in range()] for _ in range()]  # Matrix(bsize, kp)
            ...
            gQ_adj_gmean = [[TP() for _ in range()] for _ in range()]  # Matrix(kp, m3)
            for i in range(kp):
            ...
            gQ_scaled = [[TP() for _ in range()] for _ in range()]  # Matrix(kp, m3)
    ...
    V = [[TP() for _ in range()] for _ in range()]  # Matrix(k, n1)
    ...
    if (os.path.exists(get_cache_path(mpc.pid, "eigen"))):
        ...
        Z_gram = [[TP() for _ in range()] for _ in range()]  # Matrix(kp, kp)
        for i in range(kp):
    ...
    Vp = mpc.beaver_mult_vec(V, V_mask, pheno, pheno_mask)
    Vp = mpc.beaver_reconstruct(Vp)
    Vp, Vp_mask = mpc.beaver_partition(Vp)
    
    VVp = mpc.beaver_mult(Vp, Vp_mask, V, V_mask)
    ...
    V_sum = [TP(0) for _ in range()]  # Vector([Tp()  # Zp(0, param.BASE_P) for _ in range(k + param.NUM_COVS)])
    V_sum_mask = [TP(0) for _ in range()]  # Vector([Tp()  # Zp(0, param.BASE_P) for _ in range(k + param.NUM_COVS)])
    for i in range(k + param.NUM_COVS):
        for j in range(n1):
            V_sum[i] += V[i][j]
            V_sum_mask[i] += V_mask[i][j]

    u = mpc.beaver_mult(V_sum, V_sum_mask, V, V_mask)
    ...
    sx = [TP(0) for _ in range()]  # Vector([Tp()  # Zp(0, base=param.BASE_P) for _ in range(m2)])
    sxx = [TP(0) for _ in range()]  # Vector([Tp()  # Zp(0, base=param.BASE_P) for _ in range(m2)])
    sxp = [TP(0) for _ in range()]  # Vector([Tp()  # Zp(0, base=param.BASE_P) for _ in range(m2)])
    B = [[TP() for _ in range()] for _ in range()]  # Matrix(k + param.NUM_COVS, m2)
    ...
    if (os.path.exists(get_cache_path(mpc.pid, "gwas_stats"))):
        ...
        with open(get_cache_path(mpc.pid, "input_geno")) as f:
            ...
            dosage = [[TP() for _ in range()] for _ in range()]  # Matrix(bsize, m2)
            dosage_mask = [[TP() for _ in range()] for _ in range()]  # Matrix(bsize, m2)

            u_vec = [TP(0) for _ in range()]  # Vector([Tp()  # Zp(0, param.BASE_P) for _ in range(bsize)])
            u_mask_vec = [TP(0) for _ in range()]  # Vector([Tp()  # Zp(0, param.BASE_P) for _ in range(bsize)])
            p_hat_vec = [TP(0) for _ in range()]  # Vector([Tp()  # Zp(0, param.BASE_P) for _ in range(bsize)])
            p_hat_mask_vec = [TP(0) for _ in range()]  # Vector([Tp()  # Zp(0, param.BASE_P) for _ in range(bsize)])

            V.transpose(inplace=True)  # n1-by-(k + NUM_COVS)
            V_mask.transpose(inplace=True)

            V_sub = [[TP() for _ in range()] for _ in range()]  # Matrix(bsize, k + param.NUM_COVS)
            V_mask_sub = [[TP() for _ in range()] for _ in range()]  # Matrix(bsize, k + param.NUM_COVS)
            ...
            for cur in range(n1):
                ...
                g = [[TP() for _ in range()] for _ in range()]  # Matrix(3, m2)
                miss = [TP(0) for _ in range()]  # Vector([Tp()  # Zp(0, param.BASE_P) for _ in range(m2)])
                g_mask = [[TP() for _ in range()] for _ in range()]  # Matrix(3, m2)
                miss_mask = [TP(0) for _ in range()]  # Vector([Tp()  # Zp(0, param.BASE_P) for _ in range(m2)])
                ind2: int = 0
    ...
    sx_mask = mpc.beaver_partition(sx)
    sp_mask: Zp = mpc.beaver_partition(sp)

    sp2 = Tp()  # Zp(0, param.BASE_P)
    spsx = [TP(0) for _ in range()]  # Vector([Tp()  # Zp(0, param.BASE_P) for _ in range(m2)])
    sx2 = [TP(0) for _ in range()]  # Vector([Tp()  # Zp(0, param.BASE_P) for _ in range(m2)])
    ...
    numer = [TP(0) for _ in range()]  # Vector([Tp()  # Zp(0, param.BASE_P) for _ in range(m2)])
    denom = [TP(0) for _ in range()]  # Vector([Tp()  # Zp(0, param.BASE_P) for _ in range(m2 + 1)])
    if mpc.pid > 0:
        numer = sxp - spsx
        for i in range(m2):
            denom[i] = sxx[i] - sx2[i]
    ...
    denom1_sqrt_inv = [TP(0) for _ in range()]  # Vector()
